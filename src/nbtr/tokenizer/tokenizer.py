...
class Tokenizer:
    ...
    def encode_all(self, dataset, value_key):
        ...
        return dataset.map(lambda example: self.encode(example[value_key]), batched=True, batch_size=500, remove_columns=columns)
    
    def encode_ds_from_hub(self, dataset_repo_id, data_dir, filename_prefix, shard_size=None, value_key="text"):
        ...
        assert "train" in ds, "Dataset does not contain split 'train'!"
        assert "val" in ds, "Dataset does not contain split 'val'!"
        
        for split, dset in ds.items():
            try:
                if shard_size:
                    self.encode_training_data_shards(dset, split, data_dir, filename_prefix, shard_size, value_key)
                else:
                    self.encode_training_data(dset, split, data_dir, filename_prefix, value_key)
            except Exception as e:
                print("Error (encode_ds_from_hub):"+str(e))
            
    def encode_limited_stream_ds_from_hub(self, dataset_repo_id, dataset_repo_name, split, data_dir, prefix, limit, shard_size=10**8, value_key="text"):
        ds = load_dataset(dataset_repo_id, name=dataset_repo_name, split=split, streaming=True)
            
        try:
            print(ds, split ,data_dir, prefix, limit, shard_size, value_key)
            self.encode_training_data_shards(ds, split ,data_dir, prefix, limit, shard_size, value_key)
        except Exception as e:
            print("Error (encode_limited_stream_ds_from_hub):"+str(e))

    def encode_training_data(self, dset, split, data_dir, prefix="", value_key="text"):
        ...
        filename = os.path.join(data_dir, f'{prefix}{split}.bin')
    ...
    def encode_training_data_shards(self, dset, split, data_dir, prefix="", limit=None, shard_size=10**8, value_key="text"):
        if not os.path.exists(data_dir):
            os.makedirs(data_dir)
        ...
        with mp.Pool(nprocs) as pool:
            ...
            for encoded in pool.imap(encode_text_key, dset, chunksize=16):
                ...
                if token_count + len(tokens) < shard_size:
                    ...
                    filename = os.path.join(data_dir, f"{prefix}{split}_{shard_index:06d}.bin")
                ...
                if limit is not None and total_token_count>=limit:
                    break
