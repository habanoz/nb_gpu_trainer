...
class HFBackedTrainer(Trainer):
    ...
    def __init__(self, hf_trainer_config:HfTrainerConfig) -> None:
        trainer_config = hf_trainer_config.trainer_config
        assert trainer_config.repo_id is not None, "Repo id cannot be None"
        ...
        if self.api.repo_exists(repo_id=trainer_config.repo_id):
        ...
        super().__init__(config=hf_trainer_config, state=trainer_state)
