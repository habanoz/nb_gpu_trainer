...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def forward(self, position_ids):
        ...
        assert cos.dtype==torch.float32, f"cos dtype is {cos.dtype}"
        assert sin.dtype==torch.float32, f"sin dtype is {sin.dtype}"
...
class GPT(nn.Module):
    def __init__(self, config:GPTConfig):
        ...
        position_ids = torch.arange(config.seq_length, dtype=torch.float32).unsqueeze(0)
        head_dim = config.n_embed // config.n_head
        rotary_emb = LlamaRotaryEmbedding(head_dim, max_position_embeddings=config.seq_length)
    ...
    def forward(self, idx, targets=None, ignore_index=-100):
        ...
        for block in self.transformer.h:
            if self._gradient_checkpointing and self.training:
                x = gc(block,x, cos, sin, use_reentrant=True)
            else:
                x = block(x, cos, sin)
